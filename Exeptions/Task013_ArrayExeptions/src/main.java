...
public class main {
    ...
    private static int calcArray(String[][] a) throws MyArraySizeException {
        if (a.length != 4)
            throw new MyArraySizeException("Неверное количество строк: " + a.length);

        for (int i = 0; i < a.length; i++) {
            if (a[i].length != 4)
                throw new MyArraySizeException(String.format("Неверное количество элементов %d в %d-й" + " строке",
                        a[i].length, i));
        }
            ...
            for (int j = 0; j < a[i].length; j++) {
                try {
                    ...
                    throw new MyArrayDataException(
                            String.format("Неверный формат числа в позиции %d, %d: \"%s\"", i, j, a[i][j]));
                }
            }
        }
